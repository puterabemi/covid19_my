...
### Case Fatality Ratio (Daily)

```{r}
cfr_day <- covid19.data.all  %>%
  mutate(CFR = total_deaths/total_cases)
```


```{r, fig.width=5, fig.height=5}
plot.cfr.day <- ggplot(cfr_day, aes(x = date, y = CFR*1000)) +
  geom_line() +
  ylab("Case Fatality Rate\n(per 1,000 case per day)")
ggplotly(plot.cfr.day)
```


### Case Fatality Ratio (Monthly)
